...
export const getMovies = (): AppThunk => async (dispatch) => {
  try {
    ...
    const movieDetailsPromises = response.data.results.map(
      async (movie: Movie) => {
        ...
        const imdbRating: number =
          parseFloat(
            movieExtraDetails.data.Ratings.find(
              (rating: { Source: string }) =>
                rating.Source === "Internet Movie Database"
            ).Value.slice(0, 3)
          ) * 10;

        const rottenTomatoesRating: number = parseFloat(
          movieExtraDetails.data.Ratings.find(
            (rating: { Source: string }) => rating.Source === "Rotten Tomatoes"
          ).Value.slice(0, 2)
        );

        const metacriticRating: number = parseFloat(
          movieExtraDetails.data.Ratings.find(
            (rating: { Source: string }) => rating.Source === "Metacritic"
          ).Value.slice(0, 2)
        );

        const averageRating: number = Math.floor(
          (imdbRating + rottenTomatoesRating + metacriticRating) / 30
        );
